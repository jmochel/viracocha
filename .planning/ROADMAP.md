...
- [x] Phase 2: Catalogs and Patterns (3/3 plans) — completed 2026-04-04
...
| 6 | Sync engine | Filesystem sync with direction + conflict detection | SYN-01–SYN-05 | Unit/integration tests pass for catalog→ws, ws→catalog, bidirectional conflict; hidden-path behavior documented |
...
**Goal:** Introduce subscription entities bound to a project and catalog, with full CLI management.
...
- Fields (minimum): stable **id**, **catalogName** (must match registered catalog), **sourcePath** (relative to catalog root), **workspacePath** (relative to project workspace), **direction** (`CATALOG_TO_WORKSPACE` | `WORKSPACE_TO_CATALOG` | `BIDIRECTIONAL`).
...
1. User can register a subscription linking an existing catalog to paths under an existing project.
2. List/show output supports plain and JSON mirrors of other domain commands.
3. Invalid catalog/project/paths produce actionable errors.
...
- Resolve absolute paths: `catalog.root / sourcePath` and `project.workspace / workspacePath`.
...
- Update `README.md` with workflow: register catalog → create project → add subscription → `vira sync`.

**Success criteria:**

1. End-to-end test: subscription + sync modifies workspace and/or catalog trees as expected.
...
| 2. Catalogs and Patterns | v1.0 | 3/3 | Complete | 2026-04-04 |
