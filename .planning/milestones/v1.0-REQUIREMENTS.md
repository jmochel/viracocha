...
- [x] **PUB-01**: User can run `vira catalog register --name <name> --path <path>` to register a named catalog in central config
- [ ] **PUB-02**: `vira catalog register` validates that the specified path exists on disk before registering
- [x] **PUB-03**: User can run `vira catalog list` to display all registered catalogs (name, path)
- [ ] **PUB-04**: User can run `vira catalog show --name <name>` to display details of a specific catalog
- [ ] **PUB-05**: User can run `vira catalog unregister --name <name>` to remove a catalog from central config
...
- [x] **PROJ-03**: User can run `vira project add-mapping --project <name> --pattern <pattern-name> --workspace <rel-path> [--param key=value ...]` to add a mapping to a project
...
- [ ] **GEN-05**: `vira generate` skips files that already exist in the workspace (skip-existing semantics; never overwrites)
...
- **PUB-v2-01**: `vira catalog list` shows subscriber count per catalog
- **PAT-v2-01**: `vira pattern unregister` warns if pattern is referenced by existing project mappings

### Subscriptions

- **SUB-01**: User can add a subscription to a project linking a catalog source path to a workspace path
- **SUB-02**: `vira sync --project <name>` performs a one-time copy/sync of subscribed catalog files to workspace
...
| Feature | Reason |
|---------|--------|
| Remote catalogs (HTTP/Git URLs) | Local paths only in v1; adds significant network/auth complexity |
