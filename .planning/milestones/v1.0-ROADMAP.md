...
- [x] **Phase 2: Catalogs and Patterns** - Register/list/show/unregister for catalogs and patterns, Freemarker param extraction (completed 2026-04-04)
...
**Goal**: Users can register named catalogs and patterns, with Freemarker variables automatically extracted at registration time
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  ...
  2. User can list all registered catalogs and see each name and path; unregistering removes it from the list
  ...
  5. Registering a catalog or pattern with a non-existent path prints a clear error and does not modify config
**Plans**: 3 plans

Plans:
- [x] 02-01-PLAN.md — Add freemarker dep, create CatalogEntry + PatternEntry POJOs, upgrade ViracochaConfig typed lists, wire CatalogCommand + PatternCommand stubs into ViracochaCommand
- [x] 02-02-PLAN.md — Implement all four catalog leaf commands (register/list/show/unregister) with tests (PUB-01 to PUB-05)
...
**Success Criteria** (what must be TRUE):
  ...
  2. User can add a mapping to a project referencing an existing pattern, with a workspace relative path and optional key=value parameters
...
| 2. Catalogs and Patterns | 3/3 | Complete   | 2026-04-04 |
