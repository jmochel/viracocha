...
CLI commands to create, list, show, and unregister **projects** (named workspace roots), and to attach **mappings** from each project to registered **patterns** with a workspace relative path and optional per-mapping parameter values. Validates pattern references against central config before persisting. No Freemarker expansion or filesystem writes to the workspace — that is Phase 4.
...
- **D-01:** Command names and flags follow `.planning/REQUIREMENTS.md` exactly: `project create --name <n> --path <p>`, `project list`, `project add-mapping --project <name> --pattern <pattern-name> --workspace <rel-path> [--param key=value ...]`, `project show --name <name>`, `project unregister --name <name>`.
...
- **D-04:** `project show` — multi-line key-value block: project name, absolute workspace path, then each mapping (pattern name, workspace path, parameters); optional `--json` for machine-readable output consistent with Phase 2 D-03.
...
- **D-10:** Each mapping references a **pattern name** (string), a **workspace** relative path (string under project workspace), and a **parameters** map (string → string) for per-mapping values; keys must be valid for YAML and for downstream Freemarker (Phase 4).
...
- Whether the workspace path is normalized (e.g., trim, `/` vs OS separator) as long as behavior is consistent and tested.
